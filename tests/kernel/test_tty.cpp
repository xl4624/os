...
TEST(tty, color_persists_across_newline) {
  terminal_clear();
  const uint8_t color = VGA::entry_color(VGA::Color::Green, VGA::Color::Black);
  terminal_set_color(color);
  terminal_putchar('A');
  terminal_putchar('\n');
  terminal_putchar('B');
...
TEST(tty, csi_sgr_foreground) {
  ...
  ASSERT_EQ(vga_color_at(0, 0), VGA::entry_color(VGA::Color::Green, VGA::Color::Black));
}
...
TEST(tty, csi_sgr_background) {
  ...
  ASSERT_EQ(vga_color_at(0, 0), VGA::entry_color(VGA::Color::LightGrey, VGA::Color::Red));
}
...
TEST(tty, csi_sgr_bright) {
  ...
  ASSERT_EQ(vga_color_at(0, 0), VGA::entry_color(VGA::Color::LightGreen, VGA::Color::Black));
}
...
TEST(tty, csi_sgr_reset) {
  ...
  terminal_putchar('Z');
  ASSERT_EQ(vga_char_at(0, 0), 'Z');
  ASSERT_EQ(vga_color_at(0, 0), VGA::entry_color(VGA::Color::LightGrey, VGA::Color::Black));
}
