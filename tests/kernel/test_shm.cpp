...
TEST(shm, create_returns_valid_id) {
  int32_t const id = Shm::create(PAGE_SIZE);
  ASSERT(id >= 0);
...
TEST(shm, create_zero_size_fails) {
  int32_t const id = Shm::create(0);
  ASSERT_EQ(id, -1);
}

TEST(shm, create_rounds_up_to_page) {
  int32_t const id = Shm::create(1);
  ASSERT(id >= 0);
...
TEST(shm, attach_maps_pages) {
  ...
  vaddr_t const va{0x00A00000};
  int32_t const ret = Shm::attach(static_cast<uint32_t>(id), va);
  ASSERT_EQ(ret, 0);
...
TEST(shm, last_detach_frees_region) {
  ...
  ShmRegion const* region = Shm::find_region(static_cast<uint32_t>(id));
  ASSERT_NOT_NULL(region);
...
TEST(shm, unmap_nofree_preserves_physical_page) {
  ...
  paddr_t const phys = kPmm.alloc();
  ASSERT(phys != 0);

  vaddr_t const va{0x00B00000};
