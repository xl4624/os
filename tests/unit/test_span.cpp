...
TEST(span, ptr_range_construction) {
  int arr[3] = {10, 20, 30};
  std::span<int> const s(arr, arr + 3);
  ASSERT_EQ(s.size(), 3U);
  ASSERT_EQ(s[0], 10);
  ASSERT_EQ(s[2], 30);
}
...
TEST(span, const_std_array_construction) {
  ...
  std::span<const int> const s(arr);
  ASSERT_EQ(s.size(), 2U);
  ASSERT_EQ(s[0], 4);
  ASSERT_EQ(s[1], 5);
}
...
TEST(span, copy_construction) {
  int arr[3] = {1, 2, 3};
  std::span<int> const a(arr, 3);
  std::span<int> const b(a);
  ASSERT_EQ(b.size(), 3U);
...
TEST(span, const_char_span_read) {
  const char msg[] = "abc";
  std::span<const char> const s(msg, 3);
  ASSERT_EQ(s.size(), 3U);
  ASSERT_EQ(s[0], 'a');
  ASSERT_EQ(s[2], 'c');
}
