...
TEST(span, ptr_range_construction) {
  int arr[3] = {10, 20, 30};
  const std::span<int> s(arr, arr + 3);
  ASSERT_EQ(s.size(), 3U);
  ASSERT_EQ(s[0], 10);
  ASSERT_EQ(s[2], 30);
}
...
TEST(span, copy_construction) {
  int arr[3] = {1, 2, 3};
  const std::span<int> a(arr, 3);
  const std::span<int> b(a);
  ASSERT_EQ(b.size(), 3U);
...
TEST(span, const_char_span_read) {
  const char msg[] = "abc";
  const std::span<const char> s(msg, 3);
  ASSERT_EQ(s.size(), 3U);
  ASSERT_EQ(s[0], 'a');
  ASSERT_EQ(s[2], 'c');
}
