...
PhysicalMemoryManager::PhysicalMemoryManager() {
    ...
    const vaddr_t mmap_virt_end =
        phys_to_virt(paddr_t{static_cast<uintptr_t>(info->mmap_addr)}
                     + static_cast<uintptr_t>(info->mmap_length));

    const auto *mmap_base =
        phys_to_virt(paddr_t{info->mmap_addr}).ptr<multiboot_memory_map_t>();

    // Pass 1: find the highest available address to derive total_frames_.
    uint64_t highest_addr = 0;
    for (auto *e = mmap_base; vaddr_t{e}.raw() < mmap_virt_end.raw();
         e = mmap_next(e)) {
    ...
    for (auto *e = mmap_base; vaddr_t{e}.raw() < mmap_virt_end.raw();
        ...
        if (e->type == MULTIBOOT_MEMORY_AVAILABLE
            ...
            mark_free_range(paddr_t{static_cast<uintptr_t>(e->addr)},
    ...
    if ((info->flags & MULTIBOOT_INFO_MODS) && info->mods_count > 0) {
        ...
        for (uint32_t i = 0; i < info->mods_count; ++i) {
            const paddr_t mod_start =
                paddr_t{static_cast<uintptr_t>(mods[i].mod_start)}
                & ~paddr_t{PAGE_SIZE - 1};
