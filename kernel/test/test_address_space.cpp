...
TEST(address_space, map_multiple_pages) {
    ...
    for (int i = 0; i < N; ++i) {
        paddr_t page = kPmm.alloc();
        ASSERT_NE(page, 0u);
        AddressSpace::map(pd, static_cast<vaddr_t>(0x00400000 + i * PAGE_SIZE),
                          page, /*writeable=*/true, /*user=*/true);
    }
    for (int i = 0; i < N; ++i) {
        ASSERT_TRUE(AddressSpace::is_user_mapped(
            pd, static_cast<vaddr_t>(0x00400000 + i * PAGE_SIZE),
            /*writeable=*/false));
    }
    AddressSpace::destroy(pd, pd_phys);
}
