...
ScaleResult calc_scale(uint32_t fb_w, uint32_t fb_h, uint32_t src_w, uint32_t src_h) {
  uint32_t const scale_x = fb_w / src_w;
  uint32_t const scale_y = fb_h / src_h;
  uint32_t scale = (scale_x < scale_y) ? scale_x : scale_y;
  scale = std::max<uint32_t>(scale, 1);

  uint32_t const dst_w = src_w * scale;
  uint32_t const dst_h = src_h * scale;
  uint32_t const off_x = (fb_w > dst_w) ? (fb_w - dst_w) / 2 : 0;
  uint32_t const off_y = (fb_h > dst_h) ? (fb_h - dst_h) / 2 : 0;
