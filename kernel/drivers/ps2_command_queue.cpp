...
bool PS2CommandQueue::send_command(PS2Command command) {
  const PS2CommandEntry entry{
      .command = command, .parameter = 0, .has_parameter = false, .retry_count = 0};
...
bool PS2CommandQueue::send_command_with_param(PS2Command command, uint8_t parameter) {
  const PS2CommandEntry entry{
      .command = command, .parameter = parameter, .has_parameter = true, .retry_count = 0};
  if (!queue_.push(entry)) {
    return false;
  }
  if (state_ == PS2State::Idle) {
...
bool PS2CommandQueue::can_send() { return (inb(kPS2StatusPort) & kPS2StatusInputFull) == 0; }
