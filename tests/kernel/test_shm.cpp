...
TEST(shm, create_returns_valid_id) {
  const int32_t id = Shm::create(PAGE_SIZE);
  ASSERT(id >= 0);
...
TEST(shm, create_zero_size_fails) {
  const int32_t id = Shm::create(0);
  ASSERT_EQ(id, -1);
}

TEST(shm, create_rounds_up_to_page) {
  const int32_t id = Shm::create(1);
  ASSERT(id >= 0);
...
TEST(shm, attach_maps_pages) {
  ...
  const vaddr_t va{0x00A00000};
  const int32_t ret = Shm::attach(static_cast<uint32_t>(id), va);
  ASSERT_EQ(ret, 0);
...
TEST(shm, last_detach_frees_region) {
  ...
  const ShmRegion* region = Shm::find_region(static_cast<uint32_t>(id));
  ASSERT_NOT_NULL(region);
