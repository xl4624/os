...
bool load(const uint8_t* elf_data, size_t elf_len, PageTable* pd, vaddr_t& entry_out,
  ...
  for (uint16_t i = 0; i < hdr->e_phnum; ++i) {
    ...
    const vaddr_t seg_end = (ph->p_vaddr + ph->p_memsz + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
    ...
    for (vaddr_t va = seg_start; va < seg_end; va += PAGE_SIZE) {
      ...
      size_t filled = 0;
      if (va.raw() < ph->p_vaddr + ph->p_filesz && va.raw() + PAGE_SIZE > ph->p_vaddr) {
        // Byte range within the segment that this page covers.
        const size_t seg_offset = (va.raw() > ph->p_vaddr) ? va.raw() - ph->p_vaddr : 0;
        const size_t page_start = (ph->p_vaddr > va.raw()) ? ph->p_vaddr - va.raw() : 0;
        size_t copy_len = PAGE_SIZE - page_start;
        if (seg_offset + copy_len > ph->p_filesz) {
          copy_len = (ph->p_filesz > seg_offset) ? ph->p_filesz - seg_offset : 0;
        }
