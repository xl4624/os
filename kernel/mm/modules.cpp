...
void init(const multiboot_info_t* info) {
  ...
  for (uint32_t i = 0; i < n; ++i) {
    const char* name = "";
    if (mods[i].cmdline != 0) {
      const char* cmdline = phys_to_virt(paddr_t{mods[i].cmdline}).ptr<const char>();
      name = basename(cmdline);
    }

    module_table[i].name = name;
    module_table[i].data = phys_to_virt(paddr_t{mods[i].mod_start}).ptr<const uint8_t>();
...
uint32_t count() { return module_count; }
