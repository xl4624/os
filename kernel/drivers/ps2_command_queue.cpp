...
bool PS2CommandQueue::send_command_with_param(PS2Command command,
                                              uint8_t parameter) {
...
bool PS2CommandQueue::is_idle() const {
    return queue_.is_empty() && state_ == PS2State::Idle;
}
...
bool PS2CommandQueue::can_send() const {
    return (inb(kPS2StatusPort) & kPS2StatusInputFull) == 0;
}
