...
void make_two_seg_elf(TwoSegElf& elf, uint32_t entry = 0x00400000, uint32_t vaddr2 = 0x00500000) {
  memset(&elf, 0, sizeof(elf));
...
TEST(elf, load_sets_entry_point) {
  ...
  vaddr_t entry = 0;
  vaddr_t brk = 0;
  ASSERT_TRUE(Elf::load(reinterpret_cast<const uint8_t*>(&elf), sizeof(elf), pd, entry, brk));
...
TEST(elf, load_rejects_kernel_space_segment) {
  ...
  vaddr_t entry = 0;
  vaddr_t brk = 0;
  ASSERT_FALSE(Elf::load(reinterpret_cast<const uint8_t*>(&elf), sizeof(elf), pd, entry, brk));
  AddressSpace::destroy(pd, pd_phys);
}
